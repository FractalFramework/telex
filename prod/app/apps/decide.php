...
static function install(){
	...
	Sql::create('decide_args',['idPoll'=>'int','uid'=>'int','position'=>'int','txt'=>'var'],1);
	Sql::create('decide_chat',['idArg'=>'int','uid'=>'int','txt'=>'var'],1);
	Sql::create('decide_valid',['idArg'=>'int','uid'=>'int','val'=>'int'],1);}
...
static function linktopoll($id,$idArg){
	...
	return href('/app/decide/'.$id.$idArg,ico('link'),'btn');}
...
static function argsResume($p){$id=$p['id']; $ret=''; 
	...
	$ra=Sql::read('id,position','decide_args','kv','where idPoll='.$id); //pr($ra);
	//votes by args [idArg]=>[[position]=>cumul]
	$cols='decide_args.id,val';
	$r=Sql::read_inner($cols,'decide_valid','decide_args','idArg','kkc','where idPoll='.$id.' and (val=1 or val=2) order by idArg');
...
static function lead_save($p){
	$id=Sql::read('id','decide_valid','v','where idArg="'.$p['idArg'].'" and uid="'.ses('uid').'"');
	if(isset($id))$p['val']=$p['val']!=$p['current']?$p['val']:'0';
	if(!isset($id))
		$p['idChat']=Sql::insert('decide_valid',[$p['idArg'],ses('uid'),$p['val']]);
	else Sql::update('decide_valid','val',$p['val'],$id);
	return self::vote($p);}

static function vote($p){$idArg=val($p,'idArg'); $pos=val($p,'position'); $kid=randid('decide');
	$vote=Sql::read('val','decide_valid','v','where idArg="'.$idArg.'" and uid="'.ses('uid').'"');
	//tots
	$yes=Sql::read('count(val)','decide_valid','v','where idArg="'.$idArg.'" and val="1"');
	$no=Sql::read('count(val)','decide_valid','v','where idArg="'.$idArg.'" and val="2"');
...
static function chatDel($p){
	...
	if($p['idChat'])Sql::delete('decide_chat',$p['idChat']);
	return self::chatPane($p);}

static function chatSave($p){
	$p['idChat']=Sql::insert('decide_chat',[$p['idArg'],ses('uid'),$p['text']]);
	return self::chatPane($p);}
...
static function chatPane($p){$id=$p['id']; $idArg=$p['idArg']; $kid=randid('chat'); $ret='';
	...
	$r=Sql::read_inner($cols,$b,'login','uid','rr','where '.$b.'.idArg='.$idArg.' order by '.$b.'.id desc');
...
static function argumentDel($p){$idArg=$p['idArg'];
	...
	Sql::delete('decide_args',$idArg);
	Sql::delete('decide_valid',$idArg,'idArg');
	return self::play($p);}

static function argumentSave($p){$id=$p['id'];
	$r=[$id,ses('uid'),$p['position'],$p['text']];
	$p['idArg']=Sql::insert('decide_args',$r);
...
static function argumentPane($p){$ret=''; $bt='';
	...
	if(!isset($id) && $idArg)$id=Sql::read('idPoll','decide_args','v',$idArg);
	$cols='position,name,txt,decide_args.up as date'; $w='where decide_args.id='.$idArg;
	$r=Sql::read_inner($cols,'decide_args','login','uid','ra',$w);
	$rt=['1'=>'argYes','2'=>'argNo'];
	...
	$n=Sql::read('count(id)','decide_chat','v','where idArg="'.$idArg.'"');
...
static function arguments($p){$id=$p['id']; $ret='';//todo
	$r=Sql::read('id','decide_args','rv','where idPoll='.$id.' order by id');
...
static function play($p){
	...
	$r=Sql::read_inner($cols,self::$db,'login','uid','ra',$where);
	if($r['cl']==1)
	...
	//args
	$n=Sql::read('count(id)','decide_args','v','where idPoll="'.$id.'"');
