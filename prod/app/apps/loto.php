...
static function participate($p){
	$id=val($p,'id'); $val=val($p,'val'.$id); $p['label']=$id.':'.self::$a;
	if(strpos($val,'0')!==false)return self::play($p);
	$uid=Sql::read('uid',self::$db,'v',$id);//payer
	$lbl=$id.':'.self::$a;
	$ok=bank::payment(['value'=>self::$price,'type'=>self::$ty,'at'=>$uid,'label'=>$lbl]);
	if(!is_numeric($ok))return $ok;//error
	else Sql::insert(self::$db2,['bid'=>$id,'uid'=>ses('uid'),'val'=>$val,'bet'=>self::$price]);//'bk'=>$ok
	return self::play($p);}
...
static function paywin($p){$id=val($p,'id'); $price=val($p,'price');
	$uid=Sql::read('uid',self::$db,'v',$id);//payer
	$ex=Sql::insert(self::$db3,['bid'=>$id,'uid'=>ses('uid'),'price'=>$price]);
	$rb=['uid'=>$uid,'label'=>$id.':'.self::$a,'value'=>$price,'type'=>self::$ty,'at'=>ses('uid')];
	$ok=bank::payment($rb);}

static function results($r,$ex,$id){$val='';
	if($ex)$val=Sql::read('val',self::$db2,'v',$ex);//choice
	$players=Sql::read('count(id)',self::$db2,'v',['bid'=>$id]);
	$winners=Sql::read_inner('name',self::$db2,'login','uid','rv',['bid'=>$id,'val'=>$r['result']]);
	...
	if($val && $val==$r['result']){
		$payed=Sql::read('id',self::$db3,'v',['bid'=>$id,'uid'=>ses('uid')]);
