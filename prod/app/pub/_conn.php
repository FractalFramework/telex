...
class _conn{
	...
	static function injectJs(){
		return '
		function connread(){
			ajaxCall("div,conn,2|Conn,load","app=_conn,mth=reader","msg");}';}
	...
	//sample
	static function ex(){
		return '[Div*btn:div][Span*btn:span] [Italic:i] [Bold:b] [http://ph1.net] [[http://ph1.net*Url:a]*btn:span]
...
';}
	...
	static function content($p){
		...
		$p['app']='_conn'; $p['mth']='reader';//use local connectors
		$ret=self::edit('msg');
		$ret.=hlpbt('connectors');
		$ret.=tag('textarea',array('id'=>'msg','cols'=>'100%','rows'=>'10','onkeyup'=>'connread()'),$p['msg']);
		//param (app=_conn,mth=reader) will use local connectors instead of default
		$ret.=ajax($p['rid'],'Conn,load','app=_conn,mth=reader','msg',lang('convert'),'btn').br();
