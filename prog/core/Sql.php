...
#read
static function qr($sql,$z=''){if($z)echo $sql;
...
static function query($sql,$p,$z=''){if($z)echo $sql; $rq=self::qr($sql,$z);
	if($rq){$ret=self::sqlformat($rq,$p); mysqli_free_result($rq); return $ret;}}
...
static function insert2($b,$r,$o='',$z=''){
	...
	$rq=self::qr($sql,$z); return mysqli_insert_id(self::$dbq);}
...
static function update($b,$d,$v,$id,$col='',$z=''){$col=$col?$col:'id';
	self::qr('update '.$b.' set '.$d.'="'.self::escape($v).'" where '.$col.'="'.$id.'"',$z);}
static function updates($b,$r,$id,$col='',$z=''){$com=''; $col=$col?$col:'id';
	foreach($r as $k=>$v)$rb[]=$k.'="'.self::escape($v).'"';
	self::qr('update '.$b.' set '.implode(',',$rb).' where '.$col.'="'.$id.'"',$z);}
static function insertif($b,$r,$z=''){
	$ex=Sql::read('id',$b,'v',$r); if(!$ex)self::insert($b,$r);}
...
static function backup($b,$d=''){$bb='z_'.$b.'_'.$d;
	if(self::exists($bb))self::qr('drop table '.$bb);
	self::qr('create table '.$bb.' like '.$b);
...
static function exists($b){
	$rq=self::qr('show tables like "'.$b.'"');
	return mysqli_num_rows($rq)>0;}

static function columns($b,$o=''){$rq=self::qr('select COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS where table_name="'.$b.'";');
