...
static $private='1';
static $db='profile';
static $default_clr='#f5f5f5';

//install
static function install(){
Sql::create(self::$db,array('puid'=>'int','pusr'=>'var','pname'=>'var','status'=>'var','clr'=>'var','avatar'=>'var','banner'=>'var','web'=>'var','gps'=>'var','location'=>'var','privacy'=>'int','oAuth'=>'var','ntf'=>'int'),1);}
...
static function avatar_edit($p,$big){
	...
	$ret.=aj('avt|profile,avatar_save|usr='.$usr.',big='.$big.'|urlim',langp('save'),'btsav').' ';
...
static function status_edit($p){
	...
	$ret.=aj('prfl|profile,status_save|usr='.$p['pusr'].'|id,pname,status,web,gps,clr',langp('save'),'btsav');
	return $ret;}
...
static function gps($r){$ret='';
	if($r['gps'] && $r['location'])
		$ret=popup('map,com|coords='.$r['gps'],pic('location').$r['location'],'grey');
...
static function mail_edit($p){
	if(val($p,'sav'))Sql::update('login','mail',$p['mail'],ses('uid'));
...
static function deleteaccount($p){$ret=''; //pr($p);
	...
	elseif(!$open)$ret.=aj($prm.',restore=1',langp('restore account'),'btdel');
...
static function edit($p){
	$cols='id,puid,pusr,pname,status,clr,avatar,banner,web,gps,location,privacy,oAuth,ntf';
	...
	$ret.=div(self::gps($r).aj('prfloc|profile,gpsav',pic('delete')),'','prfloc');
	$ret.=tag('h2','',lang('mail'));
	$ret.=div(self::mail_edit($r),'','prml');
	$ret.=tag('h2','',lang('notifications'));
	$ret.=div(self::ntfbt($r),'','prnt');
...
static function datas($usr){
	$cols='puid,pusr,pname,status,clr,avatar,banner,web,gps,location,privacy,oAuth,ntf';
	return Sql::read($cols,self::$db,'ra','where pusr="'.$usr.'"');}

static function small($p){$usr=val($p,'usr');
	$r=self::datas($usr);
	//$r['cntban']=telex::avatar($r);
...
static function read($p){
	...
	if(!$r['clr'])$r['clr']=sesif('clr'.$usr,self::$default_clr);
...
static function create($usr){$uid=ses('uid');
	...
	if(!$id && $uid){
		...
		$r=['puid'=>ses('uid'),'pusr'=>$usr,'pname'=>$usr,'status'=>'','clr'=>$clr,'avatar'=>'','banner'=>'','web'=>'','gps'=>'','location'=>'','privacy'=>'','oAuth'=>keygen::build(),'ntf'=>''];
