...
class poll{
	...
	static function headers(){
		Head::add('csscode','
	.anscnt{margin:4px 0; padding:2px;}
	...
	static function del($p){$closed=val($p,'closed');
		...
		elseif($p['idPoll'] && $closed==1)Sql::update('poll_lead','closed','0',$p['idPoll']);
		elseif($p['idPoll'])Sql::update('poll_lead','closed','1',$p['idPoll']);//close
		if(val($p,'mnu'))return self::com($p);
		return self::pol($p);}
	...
	static function save($p){
		...
		if(val($p,'mnu'))return self::com($p);
		if($nid)return self::pol($p);}
	
	#add
	static function create($p){$nb=val($p,'nb',2); $rid=val($p,'rid');
		$inp[]='text'; for($i=1;$i<=$nb;$i++)$inp[]='answ'.$i; $inps=implode(',',$inp);
		$ret=span(aj('pllscnt,,x|poll,save|mnu=1,rid='.$rid.'|'.$inps,lang('save'),'btsav'),'right');
	...
	static function vote($p){
		$idPoll=$p['idPoll'];
		$idVote=Sql::read('id','poll_valid','v','where idPoll="'.$idPoll.'" and uid="'.ses('uid').'"');
		if(isset($idVote))$p['val']=$p['val']!=$p['current']?$p['val']:'0';
		if(!isset($idVote))
			$p['idVote']=Sql::insert('poll_valid',array($idPoll,ses('uid'),$p['val']));
		else Sql::update('poll_valid','val',$p['val'],$idVote);
		return self::read($p);}
	
	static function read($p){$idPoll=$p['idPoll']; $mnu=val($p,'mnu'); $ret='';
		$date=val($p,'date'); $answers=val($p,'answ'); if(!$date)list($answers,$date)=Sql::read('answ,UNIX_TIMESTAMP(up) as date','poll_lead','rw','where id='.$idPoll);
		$vote=Sql::read('val','poll_valid','v','where idPoll="'.$idPoll.'" and uid="'.ses('uid').'"');
		$rs=Sql::read('val','poll_valid','k','where idPoll="'.$idPoll.'" order by val');//all votes
		$sum=array_sum($rs);
		...
		//vote button
		$com='p'.$idPoll.',,,1|poll,vote|mnu='.$mnu.',idPoll='.$idPoll.',current='.$vote;
		//answ
		for($i=1;$i<=$nb;$i++){
			$answ=val($rb,$i);
			$score=val($rs,$i,0);
		...
		$ret.=div($state,'grey');
		return div($ret,'','p'.$idPoll);}
	
	#poll
	static function pol($p){$bt=''; $id=$p['idPoll']; $mnu=val($p,'mnu'); $closed=val($p,'closed');
		...
		/**/if($r['name']==ses('user') && auth(4)){
			...
			if(auth(6)){$prm='pol'.$id.'|poll,del|rid='.val($p,'rid').',mnu='.$mnu.',idPoll='.$id.',closed=';
			if(!$closed)$bt.=aj($prm.'1',lang('close'),'btdel');
			else{$bt.=aj($prm.'0',lang('open'),'btdel');
	...
	static function polls(){$ret='';
		...
		if($r)foreach($r as $k=>$v)$ret.=self::pol(array('idPoll'=>$k,'closed'=>$v));
		return $ret;}
	...
	static function call($p){$id=val($p,'id');
		...
		$ret=div(nl2br($r['txt']),'board'); $r['idPoll']=$id;
		$ret.=self::read($r);
	...
	static function com($p){$rid=val($p,'rid');
		...
		if($id)$ret.=self::pol($p);
		elseif($mnu)$ret.=self::menu($p);
		elseif($edt)$ret.=div(self::create($p),'pane','newpoll');
	...
	static function content($p){$ret='';
		...
		if(ses('uid'))
			$ret.=aj('pagup|poll,modif',pic('plus').' '.lang('new'),'btn').br().br();
		//$ret.=hlpbt('poll_app').' ';
		//root
		if(isset($p['idPoll']))$res=self::pol($p);
		else $res=self::polls();
