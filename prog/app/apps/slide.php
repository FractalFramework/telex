...
class slide{
	...
	static function create($p){$rid=val($p,'rid');
		$ret=input('inpid','');
		$ret.=ajax($rid,'slide,createsav','rid='.$rid,'inpid',langp('save'),'btn');
	return $ret;}
	...
	static function del($p){$id=val($p,'tid'); $ok=val($p,'ok');
		if(!$ok)return ajax(val($p,'rid'),'slide,del',prm($p).',ok=1','',langp('del').': '.$id,'btdel').' ';
		elseif($id && auth(6))
	...
	return ajax($rid,'slide,callin','tid='.$tid.',pid='.$pid.',rid='.$rid,'',lang('saved'),'btn');}
	...
	static function add($p){$ret=''; $rid=val($p,'rid');
		...
		$ret.=ajax($rid.'sav,,x','slide,addsav','tid='.$tid.',pid='.$pid.',rid='.$rid,$cols,langp('save'),'btsav');
	...
	static function nav($p){$ret='';
		...
		//$ret=input('inp',$pid).' ';
		//$ret=ajax($rid,'slide,callin',$prm,'',picxt(ics('slide'),$pid),'btn').' ';//refresh
		$ret.=ajax('popup','slide,edit',$prm,'',langpi('edit'),'btn').' ';
		//if($pid==1)$ret.=ajax('popup','slide,del',$prm.',delall=1','',langpi('delete'),'btdel').' ';
		$ret.=ajax('popup','slide,del',$prm,'',langpi('del'),'btdel').' ';
		$ret.=ajax('popup','slide,add',$prm.',bid=','',langpi('add'),'btn').' ';
	return div($ret);}
	...
	static function slid($r,$p){
		...
		if(isset($ra)){
			...
			if($ra['bid'])$bt1=ajax($com,$app,$prm.$ra['bid'],'',langp('previous'),'btn').' ';
			elseif(val($p,'own')){$bt1=ajax('popup','slide,editit',$prm.$tid,'',pico('edit'),'btn');
			$bt1.=ajax($rid,'slide,del',$prm.$tid.',delall=1','',langpi('delete'),'btdel').' ';
			$bt1.=href('/app/slide/'.$tid,'#'.$tid,'btxt',1);}
			//else $bt1=span($tit,'btn');
			if($ra['rel'])$bt3=ajax($com,$app,$prm.$ra['rel'],'',langp('begin'),'btn').' ';
			if($r)foreach($r as $ka=>$va){
				if($va['rel']==$pid)$bt2=ajax($com,$app,$prm.$va['pid'],'',langp('end'),'btn').' ';
				if($va['bid']==$pid)$bt4.=ajax($com,$app,$prm.$va['pid'],'',langpi('next').' '.$va['pid'],'btn').' ';}}
		$here=ajax($rid,'slide,callin',$prm.$pid,'',picxt(ics('slide'),$pid),'btn').' ';
	...
	static function menu($p){$rid=val($p,'rid'); $ret='';
		...
		if($r)foreach($r as $k=>$v)
			$ret.=ajax($rid.',,y','slide,callin','tid='.$k.',rid='.$rid,'',pico('slide').span($v),'bicon');
		if(auth(6))$ret.=ajax($rid.',,y','slide,create','rid='.$rid,'',pic('plus').span(lang('new')),'bicon');
	return div($ret);}
	...
	static function com($p){$rid=randid('sld');
		...
		if($r)foreach($r as $k=>$v){
			...
			$op=aj('pagup|slide,call|tid='.$k,pico('slide').' '.span($v),'btn');
			$ret.=div($op.$bt,'licon');}
		return div($ret,'',$rid);}
