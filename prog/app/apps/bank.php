...
static function install($p=''){
	...
	Sql::create('bank_stock',['uid'=>'int','role'=>'int','red'=>'int','blue'=>'int','green'=>'int'],1);
	Sql::create('bank_moves',['aid'=>'int','val'=>'int','to'=>'int'],1);
	Sql::create('bank_crons',['aid'=>'int','val'=>'int','to'=>'int','at'=>'int'],1);
	Sql::create('bank_contr',['cid'=>'int','if'=>'var'],1);}
...
static function create($p){$ret='';
	$ret=div(aj(self::$cb.'|bank,add|typ=red',langp('add'),'btn red'),'coin');
	$ret.=div(aj(self::$cb.'|bank,add|typ=blue',langp('add'),'btn blue'),'coin');
	$ret.=div(aj(self::$cb.'|bank,add|typ=green',langp('add'),'btn green'),'coin');
	return div($ret,'');}
...
static function coin($typ,$n){
	...
	$ret.=div($n,'btit');
	//$ret.=aj(self::$cb.'|bank,add|typ='.$typ,langpi('add'),'btn');
	//$ret.=aj('account|bank,account|typ='.$typ,langpi('view'),'btn');
...
static function stream($p){
	$p['t']='tit';
	//$ret=self::account();
	$ret=aj(self::$cb.'|bank,account',langpi('stock'),'btn');
	$ret.=appx::stream($p);
	//$ret=self::play($p);
	return $ret;}
