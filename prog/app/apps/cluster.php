...
static function install($p=''){
	...
	Sql::create('cluster_parents',['bid'=>'int','pid'=>'int','utility'=>'var','aval'=>'int'],1);
	//type:buy/use, comity id
	//Sql::create('cluster_parents',['cid'=>'int','comid'=>'int','type'=>'var','eval'=>'int'],1);
	}

static function admin($rid=''){
	$p['rid']=$rid; $p['o']=1; $p['ob']=1;
	return appx::admin($p);}
...
static function mdf_prop($p){$id=val($p,'idp');
	if(val($p,'del'))Sql::delete('cluster_prop',$id);
	else Sql::update('cluster_prop','prop',val($p,'prop'),$id);
	return self::properties($p);}

static function edt_prop($p){$id=val($p,'id'); $typ=$p['typ'];
	$r=Sql::read('distinct(attr)','cluster_attr','rv','where typ="'.$typ.'"');
...
static function sav_prop($p){$id=val($p,'id'); $unid=0; //p($p);
	...
	if($unit){
		$ex=Sql::read('id','cluster_unit','v','where unit="'.$unit.'" and atid="'.$atid.'"');
		if($ex)$unid=$ex;
		else $unid=Sql::insert('cluster_unit',['atid'=>$atid,'unit'=>$unit]);}
	$r=['bid'=>$id,'atid'=>$atid,'unid'=>$unid,'prop'=>$prop]; //p($r);
	$ex=Sql::read('id','cluster_prop','v','where atid="'.$atid.'" and bid="'.$id.'"');
	if($prop && !$ex)Sql::insert('cluster_prop',$r);
	return self::properties($p);}

static function unit($p){$id=val($p,'id'); $atid=val($p,'atid');
	$r=Sql::read('unit','cluster_unit','rv','where atid="'.$atid.'"'); //p($r);
	$ret=datalist('addunit',$r,val($p,'unit'),20,lang('referential'));
	return $ret;}

static function add_prop($p){$id=val($p,'id'); $attr=val($p,'addattr');
	$rb=Sql::read('distinct(prop)','cluster_prop','rv','');
...
static function copyfrom($p){//p($p);
	...
	if(!$bid){$r=Sql::read('id,tit',self::$db,'kv','where typ="'.$typ.'"'); //p($r);
		if($r)foreach($r as $k=>$v)
			$ret.=aj(self::$cb.'tab|cluster,copyfrom|id='.$id.',bid='.$k,$v);}
	else{$r=$r=Sql::read('atid,unid,prop','cluster_prop','rr','where bid="'.$bid.'"',1); //p($r);
		if($r)foreach($r as $k=>$v)
			$rb[]=[$id,$v['atid'],$v['unid'],$v['prop']]; pr($rb);
		Sql::insert2('cluster_prop',$rb);}
	return div($ret,'list');}
...
static function sav_attr($p){$id=val($p,'id'); $attr=val($p,'addattr');
	...
	if(!$attr)return self::add_attr($p);
		...
		$ex=Sql::read('id','cluster_attr','v','where attr="'.$attr.'"');
		$ex_prop=Sql::read('id','cluster_prop','v','where atid="'.$ex.'" and bid="'.$id.'"');
		if($ex_prop)return self::add_attr($p);
		if($ex)$p['atid']=$ex;
		else $p['atid']=Sql::insert('cluster_attr',$r);
...
static function del_parent($p){//p($p);
	if($id=val($p,'del'))Sql::delete('cluster_parents',$id);
	return self::parents($p);}

static function sav_parent($p){$pid=val($p,'addparent');
	$ex=Sql::read('id','cluster_parents','v','where bid="'.$p['id'].'" and pid="'.$pid.'"');
	$r=['bid'=>$p['id'],'pid'=>$pid,'utility'=>val($p,'utility'),'aval'=>val($p,'approbation')];
	if(!$ex)$p['pid']=Sql::insert('cluster_parents',$r);
	return self::parents($p);}

static function open_parent($p){$ret='';
	$r=Sql::read('id,tit',self::$db,'kv','where uid="'.ses('uid').'"');
	if($r)foreach($r as $k=>$v)$ret.=btj($v,atjr('val',[$k,'addparent']));
	return div($ret,'list');}
...
static function parents($p){$id=$p['id']; $cb=self::$cb;
	$r=Sql::read_inner('cluster_parents'.'.id,pid,tit,utility,aval','cluster_parents',self::$db,'pid','id','where bid='.$id);
	...
	$ret=Build::table($rb,'','',1);
	$edt=aj(self::$cb.'edt|cluster,add_parent|id='.$id,langp('add dependance'),'btsav');
	return $ret.div($edt,'',$cb.'edt');}

//read
static function props($id){
	return Sql::read_inner('cluster_prop.id,attr,prop,unit','cluster_prop','cluster_attr','atid','id','left join cluster_unit on cluster_unit.id=cluster_prop.unid where bid="'.$id.'"');}
...
static function fc_typ($k,$v){
	...
	return div(radio($k,self::$usage,$v,'2'));}
...
static function menu($p){$ret='';
	//$r=Sql::read_inner('bid,tit,typ,pid','cluster_parents',self::$db,'bid','rr','where uid="'.ses('uid').'" order by '.self::$db.'.id desc'); pr($r);
	$r=Sql::query('select '.self::$db.'.id,tit,typ,pid from '.self::$db.'
	...
	//pr($rb);//pr($ra);
	$rc=self::build_recursive($rb);// pr($rc);
